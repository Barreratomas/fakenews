...
def objective(trial):
    ...
    # -------------------------------------------------------------------------
    # Definir Espacio de Búsqueda (Refinado y Profesional)
    # -------------------------------------------------------------------------
    # Rango de LR más conservador para Fine-Tuning de Transformers
    learning_rate = trial.suggest_float("learning_rate", 1e-5, 1e-4, log=True)
    
    # Batch size:  8 o 16 para estabilidad.
    ...
    # -------------------------------------------------------------------------
    # Cargar Datos y Estrategia de Muestreo
    # -------------------------------------------------------------------------
    try:
    ...
    subset_size_train = min(total_train, 6000) 
    subset_size_val = min(len(val_ds), 1000)
    
    # Mezcla determinística para consistencia entre trials
    train_ds = train_ds.shuffle(seed=SEED).select(range(subset_size_train))
    val_ds = val_ds.shuffle(seed=SEED).select(range(subset_size_val))
    ...
    class_weights = compute_class_weights(labels_array, num_labels=DEFAULT_NUM_LABELS)
    ...
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=DEFAULT_NUM_LABELS)
    ...
    target_effective_batch = DEFAULT_BATCH_SIZE
    ...
    trainer = WeightedTrainer(
        ...
        callbacks=[
            ...
            OptunaPruningCallback(trial, metric_name="eval_f1")
        ] 
    )
